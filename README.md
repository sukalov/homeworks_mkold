...
Прошу учесть, что последний коммит программы bot2_testing.py (которая отвечает на фразу фразой из слов в тех же формах) был сделан в 00:11, то есть, как мне кажется, опоздание не то чтобы существенное. Притом синхронизировал я коммит только днём следующего дня.
